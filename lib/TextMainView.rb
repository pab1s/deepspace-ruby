...
module View
  ...
  class TextMainView
    ...
    def manageMenu(message, menu)
      ...
      begin
        ...
        begin
          ...
          if (!menuCheck.has_key?(option)) then
    ...
    def mountDiscardFromHangar(operation, element)
      ...
      begin
        # Choice and mount weapons or shields until go back
    ...
    def getChoice(howMany)
      ...
      begin
        option = Integer(capture)
        if (option < Command::GOBACK.menu || option > howMany) then
          # no se ha escrito un entero en el rango permitido
          validInput = false
        end
    ...
    def discardMountedElements(element)
      ...
      begin
        # Choice and discard weapons or shields until go back
        puts @@separator
    ...
    def stationConvertsMessage()
      if @gameUI.currentEnemy.loot.efficient
        puts "Has GANADO el combate. \tAl parecer el enemigo tenía tecnología muy avanzada... ¡Ahora eres más eficiente!"
      elsif @gameUI.currentEnemy.loot.spaceCity
        puts "Has GANADO el combate. \tTu imperio espacial prospera. ¡Has fundado una ciudad espacial!"
      end
    end
